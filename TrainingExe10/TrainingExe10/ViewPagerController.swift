...
class ViewPagerController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let v:UIView = UIView(frame: CGRect(x: 0, y: (tabView?.bounds.height)!-1.0, width: self.view.bounds.width, height: 1))
        v.backgroundColor = UIColor(red: 137.0/255.0, green: 137.0/255.0, blue: 137.0/255.0, alpha: 1)
    ...
    private func setupPageTitle()
    {
        ...
        else
        {
            ...
            var totalWidth:CGFloat = 0.0
            var labelHeightNew: CGFloat = 0.0
            for eachTitle in titles
            {
                let label = UILabel()
                label.textColor = options.tabViewTextDefaultColor
                ...
                labelHeightNew = label.intrinsicContentSize().height
            ...
            if isEvenlyDistributed
            {
                ...
                {
                    titleLabelArr[i].frame = CGRectMake(CGFloat(i) * labelWidth, 0, labelWidth, labelHeightNew)
            ...
            tabView!.contentSize = CGSize(width: totalWidth, height: labelHeightNew)
            
            let shiftHeight = tabView!.frame.height/2.0 - tabView!.contentSize.height/2.0
            tabView!.contentInset.top = shiftHeight
            }
